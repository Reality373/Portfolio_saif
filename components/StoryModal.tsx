    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-5 right-5 text-paper-muted hover:text-amber transition-colors p-2 rounded-sm bg-ink-800/80 border border-ink-600 z-20"
          ...
          <div className="p-7 sm:p-10 space-y-8">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-3">
                ...
                <span className="inline-flex items-center gap-1.5 font-mono text-xs text-amber font-semibold">
                  <FaAward className="text-sm" /> {story.badge}
                </span>
                <span className="font-mono text-xs text-paper-dim">· {story.date}</span>
              </div>

              <h2 className="font-display font-semibold text-3xl sm:text-4xl text-paper mb-2 leading-tight">
                {story.title}
              </h2>
              <p className="text-trace font-mono text-sm sm:text-base">{story.subtitle}</p>
            </div>

            {/* Metrics highlight strip */}
            <div className="grid grid-cols-3 gap-4 border-y border-ink-600 py-4 bg-ink-950/40 rounded-sm px-4">
              {story.metrics.map((metric) => (
                <div key={metric.label}>
                  <div className="font-display font-semibold text-2xl sm:text-3xl text-amber">
                    {metric.value}
                  </div>
            ...
            <div className="space-y-3">
              <h3 className="font-mono text-xs uppercase tracking-wider text-trace flex items-center gap-2">
                <span>01</span> The Context &amp; Architecture
              </h3>
              <p className="text-paper-muted leading-relaxed text-sm sm:text-base">
                {story.context}
              </p>
            </div>

            {/* The Crisis / Challenge */}
            <div className="space-y-3 p-5 rounded-sm border border-amber/30 bg-amber/5">
              ...
              <p className="text-paper leading-relaxed text-sm sm:text-base">
            ...
            <div className="space-y-3">
              ...
              <div className="text-paper-muted leading-relaxed text-sm sm:text-base space-y-3">
            ...
            {story.snippet && (
              <div className="space-y-2">
                ...
                <div className="bg-ink-950 p-4 rounded-sm border border-ink-600 overflow-x-auto">
            ...
            {/* Key Takeaway box */}
            <div className="p-5 rounded-sm border border-trace/30 bg-trace/5">
              <h4 className="font-mono text-[11px] uppercase tracking-wider text-trace mb-2 font-semibold">
                Core Engineering Principle
              </h4>
              <blockquote className="text-paper text-sm sm:text-base font-medium italic leading-relaxed">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-ink-600">
              <div className="flex flex-wrap gap-2">
                {story.tags.map((t) => (
                  <span
                    key={t}
                    className="text-xs font-mono px-2.5 py-1 border border-ink-600 text-paper-muted rounded-sm"
                  >
                    {t}
                  </span>
                ))}
              ...
              {story.relatedProjectId && (
                <a
                  href={`#projects`}
                  onClick={onClose}
                  className="inline-flex items-center gap-2 text-sm font-mono text-amber hover:underline"
                >
                  View Related Project <FaArrowRight className="text-xs" />
                </a>
              )}
            </div>
          </div>
